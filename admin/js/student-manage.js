    ...
    methods: {
        ...
        search(level) {
            ...
            axios.get(WebAPI.manage + 'student/getStudentListByAdmin?class_id=' + data.class_id + '&name=' + data.name +
                '&username=' + data.username + '&page=' + data.page + '&size=' + data.size)
                .then(function (res) {
        ...
        updateStuInfo() {
            ...
            var that = this
            axios.get(WebAPI.manage + 'student/getOneByAdmin?username=' + this.selectOptions[0].username).then(function (res) {
                that.form = res.data
            }, function (err) {
                console.log(err)
            })
        ...
        deleteStuInfo() {
            ...
            //base64加密
            usernameArr = Base64.encode(usernameArr)
            var that = this
            axios.delete(WebAPI.manage + 'student/deleteByAdmin/' + usernameArr).then(function (res) {
        ...
        beforeResetPwd() {
            ...
            this.$confirm('确认重置所选的 ' + len + ' 位学生的密码吗?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
        ...
        resetPwd() {
            ...
            var that = this
            axios.get(WebAPI.manage + 'student/resetPwdByAdmin?arr=' + arr).then(res => {
                if(res.data.code == 200){
					that.$message({
						message: '重置成功',
						type: 'success',
