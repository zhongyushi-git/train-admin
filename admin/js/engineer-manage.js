var app = new Vue({
    ...
    methods: {
        ...
        updateInfo() {
            ...
            var that = this
            axios.get(WebAPI.manage + 'engineer/getOneByAdmin?username=' + this.selectOptions[0].username).then(res => {
                that.form = res.data
            }, err => {
                console.log(err)
            })
        ...
        verify(num) {
            var that = this
            axios.get(WebAPI.manage + 'engineer/verifyByAdmin?username=' + that.selectOptions[0].username + '&flag=' + num)
			.then(res => {
                if(res.data.code == 200){
					that.$message({
						message: '审核成功',
