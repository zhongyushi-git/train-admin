var app = new Vue({
    ...
    methods: {
        ...
        updateInfo() {
            ...
            var that = this
            axios.get(WebAPI.manage + 'instructor/getOneByAdmin?username=' + this.selectOptions[0].username).then(res => {
        ...
        beforeResetPwd() {
            ...
            this.$confirm('确认重置所选的 ' + len + ' 位辅导员的密码吗?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
        ...
        resetPwd() {
            ...
            var that = this
            axios.get(WebAPI.manage + 'instructor/resetPwdByAdmin?arr=' + arr).then(res => {
                if(res.data.code == 200){
					that.$message({
						message: '重置成功',
						type: 'success',
